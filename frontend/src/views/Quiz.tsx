    ...
    return (
      <section aria-labelledby="quiz-title">
        <div className="rounded-xl border border-slate-200 bg-white p-6 text-center shadow-sm">
          <h2 id="quiz-title" className="text-2xl font-semibold">Resultados del quiz</h2>
          ...
          <p className={`my-4 text-xl font-semibold ${r.passed ? "text-green-700" : "text-red-700"}`}>
            {r.passed ? "✓ Aprobado" : "✗ Intenta de nuevo"}
          </p>
          <div className="mx-auto max-w-xl text-left">
            {r.explanations.map((exp, i) => {
              ...
              return (
                <div
                  key={exp.questionId}
                  className={`rounded-md border p-3 mb-2 ${wasCorrect ? "bg-green-50 border-green-200 text-green-800" : "bg-red-50 border-red-200 text-red-800"}`}
                >
                  <p className="m-0 mb-1 font-medium">
                    {i + 1}. {wasCorrect ? "✓" : "✗"} {qtext}
                  </p>
          ...
          <div className="mt-6 flex justify-center gap-3">
            ...
            <button type="button" onClick={() => navigate("lector", { moduleId })} className="rounded-md bg-primary-700 px-4 py-2 font-medium text-white hover:bg-primary-800">Volver al módulo</button>
          </div>
        </div>
      </section>
    );
